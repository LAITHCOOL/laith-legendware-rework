...
void EC_LocalAnimations::Instance(CUserCmd* cmd)
{
	...
	if (g_ctx.send_packet)
	{
		...
		if (bShouldSetupMatrix)
			g_ctx.local()->setup_bones_local(m_LocalData.m_aMainBones.data(), MAXSTUDIOBONES, BONE_USED_BY_ANYTHING, m_globals()->m_curtime);
		...
		g_ctx.local()->setup_bones_local(m_LocalData.m_aDesyncBones.data(), MAXSTUDIOBONES, BONE_USED_BY_ANYTHING, m_globals()->m_curtime);
...
std::array < matrix3x4_t, 128 > EC_LocalAnimations::GetDesyncMatrix()
{
	return m_LocalData.m_aDesyncBones;
}

std::array < matrix3x4_t, 128 > EC_LocalAnimations::GetLagMatrix()
{
