...
{
	/* don't overchoke */
	if (m_clientstate()->iChokedCommands >= m_NetworkData.m_nMaximumChoke)
...
{
	/* Check SignonState */
	if (!m_clientstate() || m_clientstate()->m_nSignonState < SIGNONSTATE_FULL)
	{
	...
	for (int nPlayer = 1; nPlayer <= m_clientstate()->m_nMaxClients; nPlayer++)
	{
...
void C_Networking::OnFakeDuck()
{
	...
	{
		m_NetworkData.m_nMaximumChoke = 14;
		m_NetworkData.m_nCompensatedServerTick = m_NetworkData.m_nServerTick + m_NetworkData.m_nMaximumChoke - m_clientstate()->iChokedCommands;
	}
}
void C_Networking::OnNetShowFragments(DWORD dwReturnAddress)
{
	if (!m_clientstate() || !m_clientstate()->m_NetChannel)
		return;

	uint32_t* m_NetData = &reinterpret_cast<uint32_t*>(m_clientstate()->m_NetChannel)[0x56];
	if (!m_NetData)
		return;
