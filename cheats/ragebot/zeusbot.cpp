...
void zeusbot::scan(adjust_data* record, scan_data& data)
{
	...
	{
		auto point = scan_point(record->player->hitbox_position_matrix(hitbox, record->m_Matricies[MatrixBoneSide::MiddleMatrix].data()), hitbox, true);
...
int zeusbot::hitchance(const Vector& aim_angle)
{
	...
	else
	{
		...
		for (auto i = 0; i < 256; ++i)
		{
			...
			if (hitbox_intersection(final_target.record->player, final_target.record->m_Matricies[MatrixBoneSide::MiddleMatrix].data(), final_target.data.hitbox, g_ctx.globals.eye_pos, end))
				++hits;
		}
