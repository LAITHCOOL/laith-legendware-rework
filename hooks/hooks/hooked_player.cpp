    ...
    if (player == g_ctx.local())
    {
        ...
        else
        {
            if (!g_ctx.globals.updating_animation)
            {
                ...
                local_animations::get().OnUPD_ClientSideAnims(g_ctx.local());
...
_declspec(noinline)void hooks::physicssimulate_detour(player_t* player)
{
    ...
    if (!player->valid(false, false) || !g_ctx.local()->is_alive() || player != g_ctx.local())
    ...
    engineprediction::get().store_netvars(cmd_ctx->command_number);
}
